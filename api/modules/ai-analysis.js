...
function createTextAnalysisRequest(text, userContext, model = 'gpt-5-mini') {
  return {
    model: model,
    user: `tg:${userContext.chatId}`,
    instructions: "Analyze food from text input. Output must strictly follow the JSON schema; no commentary outside JSON.\n\nSEARCH-OPTIMIZED FIELDS:\n- brand_normalized: Exact packaging brand, lowercase (m&m's, coca-cola, central lechera asturiana).\n- clean_name: 1-2 word product type WITHOUT brand (cola, chocolate bar, mantequilla, leche).\n- required_tokens: Up to 2 critical modifiers (zero, peanut, tradicional). Use [] if none.\n- off_primary_tokens: 1-3 anchor phrases in ORIGINAL LANGUAGE that must appear on the correct item.\n- off_alt_tokens: 3-8 supporting descriptors (synonyms, translations, packaging format) in lowercase.\n- off_neg_tokens: 2-8 exclusion cues (light, 0%, vegan, sin lactosa, soy) that indicate similar but wrong variants.\n- off_attr_want: ONLY if user explicitly requested specific attributes (traditional, original, classic). Use [] if not specified.\n- off_attr_avoid: Universal exclusions (light, diet, spray, low-fat, sugar-free, zero, sin lactosa, bio, vegan) that user did NOT request.\n\nCRITICAL RULES:\n- off_primary_tokens: Use EXACT original language phrase (nata montada, not whipped cream).\n- off_attr_avoid: Always include common variants user did NOT mention (light, zero, spray, sin lactosa, bio, vegan, diet).\n- off_attr_want: Only add if user explicitly said 'traditional', 'classic', 'original', 'full-fat'. Never assume.\n- Do NOT translate primary tokens - keep original language for locale consistency.\n- Keep all tokens lowercase, trimmed, no duplicates.\n- If any required value is unknown, use null (never empty strings).\n\nEXAMPLES:\n✅ 'Central Lechera Asturiana nata montada' → off_primary_tokens ['nata montada'], off_attr_avoid ['light','sin lactosa','spray','zero','diet'], off_attr_want [].\n✅ 'Coca-Cola Zero Sugar' → off_primary_tokens ['coca cola zero'], off_attr_avoid ['light','diet','cherry','vanilla'], off_attr_want ['zero','sugar free'].\n✅ 'red kidney beans in water' → off_primary_tokens ['red kidney beans'], off_attr_avoid ['chilli','refried','black beans','organic','low sodium'], off_attr_want [].\n",
